...
//the node* immediately after rightmost node of a level will be nullptr
template <typename T>
void right_view_v1(node<T> *root)
{
    if (root == nullptr)
        return;
...
template <typename T>
void right_view_v2(node<T> *root)
{
    if (root == nullptr)
        return;
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < lvl_size; i++)
        {
            ...
            if (i == lvl_size - 1) //main part of right size view
                cout << temp->data << " ";
        }
    }
}
